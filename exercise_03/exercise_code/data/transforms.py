...
class RescaleTransform:
    ...
    def __call__(self, images):
        ...
        outrange=self.max-self.min
        inrange=self._data_max-self._data_min
        difference=outrange/inrange
        for i in range(images.shape[0]):
            for j in range(images.shape[1]):
                for z in range(images.shape[2]):
                    images[i,j,z]=images[i,j,z]*difference
...
def compute_image_mean_and_std(images):
    ...
    std=[]
    for i in range(3):
        temp1=np.mean(images[:,:,:,i])
        temp2=np.std(images[:,:,:,i])
        mean.append(temp1)
        std.append(temp2)
    mean=np.array(mean)
    std=np.array(std)
